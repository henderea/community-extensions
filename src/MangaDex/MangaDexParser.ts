...
export const parseMangaList = async (object: MangaItem[], source: any, thumbnailSelector: any): Promise<PartialSourceManga[]> => {
  ...
  for(const manga of object) {
    ...
    const title = decodeHTMLEntity(mangaDetails.title.en ?? Object.values(mangaDetails.title)[0] ?? mangaDetails.altTitles.map((x) => Object.values(x).find((v) => v !== undefined)).find((t) => t !== undefined) ?? 'INVALID TITLE');
